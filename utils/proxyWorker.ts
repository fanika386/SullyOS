 ...
 *
 * 本 remix 分支默认指向用户自部署的实例（sullyos-main-proxy.leonanice36.workers.dev），
 * 不再占用作者公共实例。如果还要换，把自己部署的 worker 地址填进
 * 「设置 → 网络代理 (Worker)」即可，以上全部能力会自动切到你的实例，无需改任何代码。
 *
 * 注意：小红书 Lite 目前不单独暴露地址输入框，使用这里的主代理 Worker + /api；
 * 网易云音乐仍保留独立地址框，单独填了就以音乐配置为准。
 */

export const DEFAULT_PROXY_WORKER = 'https://sullyos-main-proxy.leonanice36.workers.dev';
...
//   - sullymeow.ccwu.cc：作者公共实例，remix 换成自部署后不再作为默认（2026-08 起）
const STALE_HOSTS = [
