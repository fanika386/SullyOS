...
// 排程接口把角色写的墙钟折成的绝对时刻。用「当前时刻往后一小时」而不是写死的日期：
// 这个值要落进任务清单，而清单会清掉过点超过 48h 的一次性任务，写死旧日期会让用例过期。
const RESOLVED_ISO = new Date(Date.now() + 60 * 60_000).toISOString();
...
describe('角色排程的时间统一存绝对时刻', () => {
  ...
  it('回话里的时间按角色的钟说，且只折一次', async () => {
    ...
    // 纽约角色写「明早九点」时，排程接口回传的绝对时刻就是 RESOLVED_ISO；
    // 回话必须按纽约墙钟说出那一刻，而不是按设备时区再折一次。
    const reply = await executeAmsg2Tool(
      'schedule_active_message',
      { send_at: RESOLVED_ISO },
      deps,
    );

    const expectedNy = new Intl.DateTimeFormat('en-CA', {
      timeZone: 'America/New_York', hour: '2-digit', minute: '2-digit', hour12: false,
    }).format(new Date(RESOLVED_ISO)).replace('24:', '00:');
    expect(reply).toContain(expectedNy);
  });
});
