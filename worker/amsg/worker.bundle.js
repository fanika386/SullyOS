...
var DEFAULT_PROXY_WORKER = "https://sullyos-main-proxy.leonanice36.workers.dev";
...
var extractXsecSourceFromUrl = (url) => {
  try {
    const u = new URL(url);
    return u.searchParams.get("xsec_source") || void 0;
  } catch {
    return void 0;
  }
};
...
var normalizeNote = (n) => {
  ...
  const xsecToken = n.xsecToken || n.xsec_token || card?.xsec_token || card?.xsecToken || void 0;
  const xsecSource = n.xsecSource || n.xsec_source || card?.xsec_source || card?.xsecSource || void 0;
  const webUrl = n.webUrl || n.web_url || n.url || (noteId ? `https://www.xiaohongshu.com/explore/${noteId}${xsecToken ? `?xsec_token=${encodeURIComponent(xsecToken)}${xsecSource ? `&xsec_source=${encodeURIComponent(xsecSource)}` : ""}` : ""}` : void 0);
  return {
    noteId,
...
// utils/xhsCapabilities.ts
var DEFAULT_XHS_CAPABILITIES = {
  search: true,
  browse: true,
  detail: true,
  myProfile: true,
...
function isBlankXhsDetailData(detailData) {
  ...
  return !title && !desc && !author && comments.length === 0 && !commentsError && (!Array.isArray(images) || images.length === 0);
}
function detailFailureMessage(detailData) {
...
async function runXhsDetail(args, ctx) {
  ...
  console.log(`\u{1F4D5} [XHS] AI\u8981\u67E5\u770B\u7B14\u8BB0\u8BE6\u60C5:`, args.noteId, xsecToken ? "(\u6709xsecToken)" : "(\u65E0xsecToken)", xsecSource ? `(source=${xsecSource})` : "");
    ...
    if (cachedTitle) {
      ...
      if (refreshResult.success && refreshResult.notes.length > 0) {
        ...
        if (refreshedNote?.xsecToken) {
          ...
          console.log(`\u{1F4D5} [XHS] \u62FF\u5230\u65B0 xsecToken\uFF0C\u91CD\u8BD5 detail...`, xsecSource ? `(source=${xsecSource})` : "");
  ...
  if (result.success && result.data && typeof result.data === "object") {
    ...
    const detailSource = noteObj?.xsecSource || noteObj?.xsec_source || d?.xsecSource || xsecSource;
